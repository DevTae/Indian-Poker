...
def indianFoker():
            ...
            if player_turn:
                ...
                if isAllIn:
                    ...
                    if player_betting_chips-com_betting_chips > com_chips:
                        behavior = selectBehavior(present, player_value, com_chips, com_chips) # 다이 or 올인
                    else:
                        behavior = selectBehavior(present, player_value, (player_betting_chips)-(com_betting_chips), (player_betting_chips)-(com_betting_chips)) # 다이 or 격차만큼 베팅
                else:
                    if (player_betting_chips)-(com_betting_chips) >= com_chips: # 칩 부족:다이 or 올인
                        ...
                        if com_chips - (player_betting_chips-com_betting_chips) - player_chips >= 0: # 칩 많음:컴퓨터가 베팅에서 손해보지 않게 손봐줌. 최댓값의 경계값을 정해줌
                            behavior = selectBehavior(present, player_value, (player_betting_chips)-(com_betting_chips), (player_betting_chips)-(com_betting_chips) + player_chips) #불필요한 베팅을 자제함.
                        else:
                            behavior = selectBehavior(present, player_value, (player_betting_chips)-(com_betting_chips), com_chips)
    ...
    if not (player_chips-1 > 0 and com_chips-1 > 0):
        ...
        #칩개수비교해서 승패가름.
        print("*카드가 모두 소진된 이유로 칩의 개수에 근거하여 승패를 가르도록 하겠습니다.\n")
        time.sleep(1)
